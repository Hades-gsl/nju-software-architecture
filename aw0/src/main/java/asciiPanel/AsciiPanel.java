...
public class AsciiPanel extends JPanel {
  ...
  /**
   * Sets the distance from the left new text will be written to. This should be equal to or greater
   * than 0 and less than the the width in characters.
   *
  ...
  /**
   * Sets the distance from the top new text will be written to. This should be equal to or greater
   * than 0 and less than the the height in characters.
   *
  ...
  /**
   * Sets the used font. It is advisable to make sure the parent component is properly sized after
   * setting the font as the panel dimensions will most likely change
   *
   * @param font
   */
  public void setAsciiFont(AsciiFont font) {
    ...
    offscreenBuffer =
        new BufferedImage(panelSize.width, panelSize.height, BufferedImage.TYPE_INT_RGB);
    ...
    for (int x = 0; x < widthInCharacters; x++) {
      for (int y = 0; y < heightInCharacters; y++) {
        ...
        if (previousCharacterData != null
            ...
            && newCharacterData.character == previousCharacterData.character) continue;
  ...
  /**
   * Create a <code>LookupOp</code> object (lookup table) mapping the original pixels to the
  ...
  /**
   * Clear the entire screen with the specified character and whatever the specified foreground and
   * background colors are.
   *
   * @param character the character to write
  ...
  public AsciiPanel clear(char character, int x, int y, int width, int height) {
    return clear(
        new AsciiCharacterData(character, defaultForegroundColor, defaultBackgroundColor),
        x,
        y,
        width,
        height);
   ...
   */
  public AsciiPanel clear(
      char character, int x, int y, int width, int height, Color foreground, Color background) {
   ...
   * colors. This updates the cursor's position but not the default foreground or background colors.
   *
   * @param character the character to write
   * @param x the distance from the left to begin writing from
   * @param y the distance from the top to begin writing from
  ...
  /**
   * Write an AsciiCharacterData to the specified position. This updates the cursor's position but
   * not the default foreground or background colors.
   *
   * @param characterData the AsciiCharacterData to write
   * @param x the distance from the left to begin writing from
   * @param y the distance from the top to begin writing from
  ...
  /**
   * Write a string to the specified position with the specified foreground color. This updates the
   * cursor's position but not the default foreground color.
   *
   * @param string the string to write
   * @param x the distance from the left to begin writing from
